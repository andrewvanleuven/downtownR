suppressPackageStartupMessages({library(tidyverse); library(SpatialKDE); library(sf); library(viridis); library(tidycensus); 
library(tigris); library(janitor); library(ggpubr); library(data.table); library(rleuven)})

# Read in data ------------------------------------------------------------
df <- fread('hidden/ny_pa_2005.csv')
univ <- fread('data/univ_nypa.csv') %>% 
  mutate(st = if_else(st == 'Pennsylvania','PA','NY'))

# Random Municipality -----------------------------------------------------
town_fips <- univ %>% filter(pop_2010 < 7500, rucc > 3) %>% sample_n(1) %>% pull(city_fips)
# town_fips <- 4256456 # Oil City, PA ==> proof of concept that a polycentric downtown district WORKS
town_name <- univ %>% filter(city_fips == town_fips) %>%
...
cnty <- counties(st, cb = T) %>% st_transform(3649) %>%
...
cell_size <- 100
band_width <- 150
bfam <- 'Blender Pro Bold'
bsize <- 9
tface <- 'plain'

# D3 Step 1 ---------------------------------------------------------------
(stp1aa <- ggplot() +
   geom_sf(data = st_make_grid(town_sf, cellsize = cell_size, square = F) %>%
             st_intersection(town_sf), alpha = 0, size = .4, color = "gray70") +
   geom_sf(data = town_sf, alpha = 0, color = 'black', size = .5) +
...
(stp1a <- ggplot() +
    ...
    geom_sf(data = st_make_grid(town_sf, cellsize = cell_size, square = F) %>%
              st_intersection(town_sf), alpha = 0, size = .4, color = "gray70") +
    geom_sf(data = town_dots_all, size = .1)+
    geom_sf(data = town_sf, alpha = 0, color = 'black', size = .5) +
    theme_void(base_family = bfam, base_size = bsize) +
...
(stp1b <- ggplot() +
    ...
    geom_sf(data = town_roads, alpha = .5, size = .2) +
...
(stp2b <- ggplot() +
    ...
    geom_sf(data = town_dots_all, size = .25, stroke = .25,
            shape = 21, fill = "white", alpha = 1, color = "black")+
    geom_sf(data = town_kde2b, aes(fill = kde_value), color = NA, alpha = .5) +
...
(stp3a <- ggplot() +
    ...
    geom_sf(data = kde_polygons2b, alpha = 0, color = "black", size = .2) +
    geom_sf(data = town_roads, alpha = .5, size = .2) +
    geom_sf(data = town_dots_all, size = .25, stroke = .01,
            shape = 21, fill = "black", alpha = .9)+
    geom_sf(data = town_sf, alpha = 0, color = 'black', size = .5) +
    scale_fill_viridis_c(direction = -1, option = "magma") +
    theme_void(base_family = bfam, base_size = bsize) +
...
(stp3b <- ggplot() +
    geom_sf(data = town_sf, fill = "gray95") +
    geom_sf(data = town_h2o, color = "black", fill = "#7BAFD4", alpha = .7, size = .2) +
    geom_sf(data = kde_blob3 %>% st_buffer(20), fill = "black", alpha = .3) +
    geom_sf(data = town_kde2b, aes(fill = kde_value), color = NA, alpha = .4) +
    geom_sf(data = kde_polygons2b, alpha = 0, color = "black", size = .2) +
    geom_sf(data = town_roads, alpha = .5, size = .2) +
    geom_sf(data = town_dots_all, size = .25, stroke = .01,
    ...
    scale_fill_viridis_c(direction = -1, option = "magma") +
    theme_void(base_family = bfam, base_size = bsize) +
    labs(title = 'Buffer Operation (50m)') +
    theme(legend.position="none",plot.title = element_text(hjust = .5, face = tface)))
...
# Plot and Save -----------------------------------------------------------
filename <- sprintf('plot/saved_grids/%s.png',(snakecase::to_snake_case(town_name)))
stps_grid <- ggarrange(stp1aa,stp1a,stp1b,stp2a,stp2b,stp2c,stp3a,stp3b,stp3c,ncol = 3, nrow = 3) +
  ggsave(filename, width = 12, height = 9)
beepr::beep(2)
